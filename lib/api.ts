...
const API_BASE_URL = "https://backend.morsl-kec.com"
...
// Legal Document API calls
export async function getLegalDocument(type?: "privacy" | "terms"): Promise<LegalDocumentResponse> {
  try {
    const params = new URLSearchParams()
    if (type) {
      params.append("type", type)
    }
    const response = await api.get<LegalDocumentResponse>(`/legal-document?${params.toString()}`)
    return response.data
  } catch (error) {
...
export async function saveLegalDocument(
  data: { privacyPolicy?: string; termsConditions?: string },
  token: string,
): Promise<LegalDocumentResponse> {
  try {
    const response = await api.post<LegalDocumentResponse>(`${API_BASE_URL}/legal-document`, data, {
      headers: {
        Authorization: token, // Send token directly without "Bearer"
        "Content-Type": "application/json",
      },
